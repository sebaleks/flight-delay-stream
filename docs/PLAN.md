...
8. **Cancelled flights are excluded from the headline metrics into their own counter (`excluded_cancelled`), never dropped and never counted.** ArrDel15 is undefined for a cancellation, so it is neither a hit nor a false alarm: silent dropping would hide join bugs, and counting a cancellation as a non-delay would penalize exactly the flights an ops audience most wants flagged. The evaluator's counters therefore split "correctly unmatched because cancelled" from "missing or late outcome" (with TTL-evicted and end-of-stream sub-counts), plus diverted-without-label, warm-up exclusions, and orphan outcomes, and the test suite asserts every event lands in exactly one category (conservation, `streaming/test_evaluator.py`). Measured basis: the week's 104 null-tail rows are all cancellations, so the sentinel partition and this category largely coincide.
...
Dress rehearsal (2026-08-13, four rounds from fresh clones): each of the first three rounds exposed a real defect that every existing gate had missed, because all of them ran against a warm machine. Round 1: the batch consumer's `subscribe()` group join raced fetch-position resolution on a seconds-old broker and every partition reported EOF with no message, so the consumer scored 0 of 151,878 — fixed by explicit partition assignment from offset 0 (commit 3116433). Round 2: `scripts/fetch_artifacts.sh` shelled out to `gh` with no `--repo`, and a clone whose remote is not a GitHub host left `gh` unable to infer the repository; phase 1 failed and the consumer ran artifact-less (commit 0cc7efb). Round 3, the one that mattered: the consumer still scored 0, and the cause was broker-side. The replay carries event-time message timestamps in September 2024 by design (CLAUDE.md section 2 forbids wall clock in the stream), and Kafka's default time-based retention judges segment age by exactly those timestamps. Every segment is roughly two years stale on arrival, so the first retention sweep after broker start deletes the entire replay week. Confirmed directly: log-start offset equal to the high watermark on all 12 partitions of both input topics. `make demo` had always passed only because produce-to-consume finished inside the sweep window; nothing in the test suite or the determinism double-run could see it, since both ran warm and fast. Fixed by disabling retention in `docker-compose.yml` (topics are recreated per demo, so disk stays bounded at one replay week), plus two defences that turn this failure class loud instead of silent: the batch consumer now requires every partition's consumed position to reach the high watermark before it accepts EOF as completion, and fails with the watermark state after bounded retries rather than reporting a short read as success; both producers now carry delivery callbacks, so a failed send can no longer be counted in "produced N".

Closeout (2026-08-13): the shipped-versus-claimed audit is DONE and finalized in the ledger above (its two open rows: the unbuilt §6 fallback, and instructor sign-off). The harmonization follow-up is MEASURED (representation dominates 89/11; retrain justified as future work only). The origin-pressure fields are BUILT: `PressureIndex` in the consumer counts late arrivals into and cancellations out of the origin over the trailing `PRESSURE_WINDOW_HOURS` = 3 event-time window, strictly before T, from the outcomes stream; semantics in `docs/schemas.md`, boundaries pinned by `streaming/test_pressure.py`. Remaining for the team: obtain instructor sign-off (ledger row), and decide whether the §6 fallback claim gets built or stays on record as unmet.
